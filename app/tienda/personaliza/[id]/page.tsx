...
export default function EstudioItemPage() {
  ...
  if (item === null) {
    return (
      <div className="max-w-lg mx-auto px-4 py-24 text-center">
        <p className="font-mono text-xs uppercase text-[var(--tienda-text)]/60 mb-4">Ítem no disponible</p>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 md:py-20">
      <Link href="/tienda/personaliza" className="font-mono text-xs uppercase text-[var(--tienda-text)]/40 hover:text-red-600 transition-colors">
        ← Volver
      </Link>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-start mt-8">
        <div className="aspect-square bg-[var(--tienda-text)]/5 border border-[var(--tienda-text)]/20 relative overflow-hidden">
          {item.imagen_url ? (
            <Image src={item.imagen_url} alt={item.nombre} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-6xl text-[var(--tienda-text)]/10">📷</div>
          )}
        </div>

        <div className="flex flex-col">
          {item.subtitulo && (
            <p className="font-mono text-xs text-[var(--tienda-text)]/40 uppercase tracking-widest mb-3">{item.subtitulo}</p>
          )}
          <h1
          ...
          {item.precio && (
            <p className="font-mono text-2xl font-bold border-t border-[var(--tienda-text)]/20 pt-6 mb-6" style={{ color: 'var(--tienda-text)' }}>
              {item.precio}
            </p>
          )}

          {item.descripcion && (
            <p className="text-sm text-[var(--tienda-text)]/60 mb-8 whitespace-pre-line leading-relaxed">{item.descripcion}</p>
          )}
