...
El repo tiene un único `origin` con dos push URLs (GitHub `SVValen/TERRA` y `SVValen/SHOWROOM`): mismo código, dos despliegues Vercel. Diferencias entre ambos se resuelven con feature flags por env var (ver `lib/features.ts`), nunca con ramas divergentes. Custom Studio está oculto en SHOWROOM vía `NEXT_PUBLIC_HABILITAR_PERSONALIZA=false`.
...
Sección de la tienda para diseño personalizado, con feature flag `personalizaHabilitado()` (`lib/features.ts`, env `NEXT_PUBLIC_HABILITAR_PERSONALIZA`, habilitada por defecto salvo que valga exactamente `'false'`). `/tienda/personaliza` (home de la sección: hero, "Solo diseño", "Diseño + producto" con ítems de `estudio_items`, "Tu prenda, nuestro diseño", CTA final) y `/tienda/personaliza/[id]` (detalle de un ítem de "Diseño + producto") son públicas; `/admin/personaliza` administra el contenido (textos e imagen vía `negocio.custom_studio` jsonb + `custom_diseno_imagen_url`) y el CRUD de ítems ilimitados de `estudio_items` (nombre, subtítulo, descripción, precio, imagen, orden, activo) vía `/api/estudio-items` (admin) y `/api/tienda/estudio-items` (público). Todas las páginas y los 4 endpoints (`/api/estudio-items`, `/api/estudio-items/[id]`, más el nav de `Header.tsx`/`Sidebar.tsx` y la sección de mensajes en `/admin/whatsapp`) respetan el flag: páginas redirigen client-side (a `/` en tienda, a `/admin/stock` en admin), endpoints devuelven 404 si está deshabilitado.
