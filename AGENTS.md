...
categorías y panel de "Interés" (no es carrito de compra), para que
...
## Deployments TERRA / SHOWROOM
El repo tiene un único `origin` con dos push URLs (GitHub `SVValen/TERRA` y `SVValen/SHOWROOM`): mismo código, dos despliegues Vercel, cada uno con su propia base Supabase. Diferencias entre ambos (por ejemplo, mostrar u ocultar Custom Studio o Misión/Visión) se resuelven con toggles en la tabla `negocio` (`personaliza_habilitado`, `mision_vision_habilitado`, editables desde el panel admin), nunca con ramas divergentes ni env vars — al ser bases de datos separadas, cada deployment puede tener el toggle en un valor distinto sin tocar código.
...
`/tienda` (home) muestra un hero full-width con los productos `destacado = true` (uno a pantalla ancha por vez, con flechas/dots, reproduciendo el `video_url` del producto en loop si se cargó uno en vez de la foto) seguido del carrousel "Nuevos" (últimos por `creado_en`, cards centradas si son pocas). El catálogo completo (búsqueda + filtros + grilla) **no vive en el home**: es un panel lateral oculto por defecto (`CatalogoSidebar.tsx`) que se abre desde el header o desde un botón "Ver catálogo completo" — se decidió así porque con pocos productos cargados una grilla fija dejaba mucho espacio vacío en desktop. El header incluye un menú desplegable de categorías (hover en desktop, acordeón full-screen en mobile) que al clickear abre directamente ese panel con el filtro correspondiente, y un ícono de "Interés" con contador. El panel de "Interés" (`PanelInteres.tsx`) es una lista de productos que la clienta arma mientras navega (persistida en `localStorage`, **no es un carrito de compra real** — no hay stock reservado ni checkout) y que se resuelve con **un solo mensaje de WhatsApp** listando todo. Cada card de producto muestra badge "Nuevo" (creado hace menos de `negocio.diasNuevo` días, default 14) y "% OFF" (si `precio_anterior > precio_venta`), con la segunda foto del producto on-hover en desktop. El detalle de cada producto (`/tienda/[id]`) suma una sección "También te puede interesar" con hasta 8 productos de la misma categoría (o los más recientes si no hay suficientes). El footer suma columnas de Ayuda (guía de talles, cambios/devoluciones y envíos — cada una abre un modal con contenido editable desde `/admin/negocio`), Institucional (`razon_social`/`cuit`/`direccion`, solo los que estén completos) y Seguinos (WhatsApp + Instagram).

### 8. Contenido editable de la tienda (guía de talles, envíos, cambios/devoluciones, Misión/Visión)
Desde `/admin/negocio` se edita: la **guía de talles** como tabla genérica (columnas y filas libres, sin estructura fija — sirve tanto para remeras como para cualquier otra prenda), el texto de **cambios y devoluciones**, el texto de **envíos**, un **banner animado** (`banner_envios`, cartel tipo cinta que corre en el header y el footer de la tienda — vacío lo oculta) y las **etiquetas de envío** ("envío gratis"/"envío en el día", texto personalizable). Estas dos últimas se activan **por producto** con los checkboxes `envio_gratis`/`envio_dia` en `/admin/stock/[id]`, y se muestran como cartelitos en la card del producto en la tienda pública. La sección **Misión y Visión** del home (texto + imagen por cada una, componente `MisionVision.tsx`) se edita en el mismo panel y tiene su propio checkbox "Mostrar en la tienda" (`negocio.mision_vision_habilitado`, default `true`) para ocultarla sin perder el contenido cargado.

### 9. Custom Studio / "Personalizá tu diseño"
Sección de la tienda para diseño personalizado, con visibilidad controlada por `negocio.personaliza_habilitado` (boolean, default `true`, checkbox "Mostrar en la tienda" en `/admin/personaliza`). `/tienda/personaliza` (home de la sección: hero, "Solo diseño", "Diseño + producto" con ítems de `estudio_items`, "Tu prenda, nuestro diseño", CTA final) y `/tienda/personaliza/[id]` (detalle de un ítem de "Diseño + producto") son públicas y redirigen a `/` si el toggle está apagado; el link "Personalizá tu diseño" del header (`Header.tsx`) se oculta en el mismo caso. `/admin/personaliza` (siempre accesible, sin depender del toggle, para poder preparar contenido de antemano) administra el toggle, los textos e imagen (vía `negocio.custom_studio` jsonb + `custom_diseno_imagen_url`) y el CRUD de ítems ilimitados de `estudio_items` (nombre, subtítulo, descripción, precio, imagen, orden, activo) vía `/api/estudio-items` (admin, sin gateo) y `/api/tienda/estudio-items` (público, devuelve 404 si el toggle está apagado).

### 10. Mensajes de WhatsApp configurables
Desde `/admin/whatsapp` se editan los textos de los mensajes que arma la tienda: saludo (`whatsapp_saludo`), intro de consulta de un producto (`whatsapp_msg_producto_intro`), intro del panel de Interés (`whatsapp_msg_interes_intro`) y, si Custom Studio está habilitado, los tres mensajes de esa sección (`whatsapp_msg_estudio_proceso`, `whatsapp_msg_estudio_general`, `whatsapp_msg_estudio_item`). `lib/whatsapp.ts` expone `sustituirTokens()` para reemplazar placeholders tipo `{tienda}`/`{item}` en esos textos antes de armar la URL de WhatsApp.
